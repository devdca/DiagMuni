...
- `.claude/agents/transformacion-digital.md` línea 10 (brief de este mismo agente): "variables de contexto (población, personal total, presupuesto TIC)... variables de capacidad institucional (área TIC, conectividad, normativa local emitida)" — confirma la misma lista de 6 variables y aclara "personal **total**", relevante para el punto 2.4 de este documento (distinguirla de "funcionarios involucrados por trámite"). Línea 15 del mismo archivo: "Diseña para el funcionario municipal promedio, no para el tecnólogo: el instrumento se responde sin conocimientos técnicos."
...
- **Tipo:** `integer`, `>= 0`, nullable.
- **Nombre de campo deliberadamente distinto de "personal" a secas**, para no confundirse con "funcionarios involucrados por trámite" (`entregables/fase-1/matriz-normativa.md` línea 31), variable ya prevista como insumo por trámite (`CLAUDE.md`, "Entrada por municipio: (a) funcionarios involucrados por trámite") y fuera del alcance de este documento. Son dos números distintos: uno es la nómina completa del gobierno (esta variable), el otro es cuántos de esos funcionarios atienden un trámite específico (variable ya existente en el diseño del cuestionario por trámite).
