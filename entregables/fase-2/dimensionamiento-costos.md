...
**Decisión vigente:** DeepSeek + Claude vía API como default de producción, con degradación a plantillas deterministas si la API no responde (ver `docs/stack-tecnologico.md`, "Nota sobre la Capa IA"). Un benchmark real comparó ambos caminos con el mismo párrafo de prueba (F3, "Firma electrónica"): la API de DeepSeek generó la respuesta en 4.7 segundos (~USD 0.0005-0.001 por llamada, ver sección 8 para la cifra exacta y su metodología) contra 101-194 segundos de los mejores candidatos locales con licencia limpia — una diferencia de un orden de magnitud en latencia, con costo marginal insignificante para el volumen de un piloto.

El wiring de código del modelo local ya está construido — no es una alternativa manual futura: `backend/app/ia/litellm_config.yaml` define la ruta `local`, y `backend/app/ia/config.py` (`obtener_proveedor_llm()`) resuelve, sin `LLM_PROVIDER` fijado explícitamente, `local` como default si `OLLAMA_API_BASE` está poblada (plantilla determinista si no). El operador de cada despliegue que quiera usar Claude/DeepSeek en producción debe fijar `LLM_PROVIDER=anthropic` o `=deepseek` a propósito. La instalación real de Ollama y la prueba de integración end-to-end sin mocks ya se ejecutaron con éxito (`backend/tests/test_generador_plan_ollama_real.py`, ver `docs/stack-tecnologico.md`). Lo que sigue en este documento dimensiona y costea el VPS necesario si ese escenario se activa en producción con volumen, para que quede listo si se necesita.
