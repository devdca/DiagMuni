...
Resultado del cierre (03-ago-2026): **3 de 6 categorías con costo de infraestructura verificado con cifra concreta** (`gestor_expediente_electronico`, `identidad_federada`, `conector_interoperabilidad`); **2 de 6 sin costo de infraestructura aplicable porque no requieren un servicio adicional que hostear** (`modulo_cifrado_datos`, `adaptador_pasarela_pago` — declarado como hecho verificado, no como hueco); **1 de 6 con costo de infraestructura `[NO VERIFICADO]`** (`modulo_firma_electronica`, porque la fuente oficial no publica una cifra concreta). **Costo de licenciamiento verificado en 0 para las 6 categorías** (todas OSS, ya confirmado en el paso 1). **Costo de implementación/capacitación `[NO VERIFICADO]` en las 6 categorías** — no se encontró ninguna fuente pública citable de horas típicas de despliegue por componente; se documenta como hueco honesto en las 6, en vez de fabricar una cifra a partir de tarifas de mercado sueltas sin un número de horas verificable que multiplicar.

**Actualización — 18 de agosto de 2026 (verificación puntual, no una tercera pasada exhaustiva):** chequeo rápido de si algo cambió en los 8 días desde la segunda pasada -- releases/documentación de `esig/dss-demonstrations` y una búsqueda de caso de uso municipal combinando Keycloak/Mayan EDMS/X-Road. Mismo resultado en ambos: ninguna cifra nueva. No se repitieron las 6 vías completas de la pasada anterior (ya agotadas); se deja como constancia de que la brecha sigue siendo estructural en esta fecha, no que se re-verificó a fondo. Sin cambios en la conclusión de las secciones 3 y 7.

**Actualización — 10 de agosto de 2026 (segunda pasada sobre los 3 huecos declarados):**
- **Cerrado:** el escalón recomendado de `gestor_expediente_electronico` (Docker Compose, 16 GB RAM/8 núcleos) ya tiene cifra verificada — ver Sección 2. `gestor_expediente_electronico` queda ahora con **ambos** tiers (mínimo y recomendado) verificados.
- **Sigue abierto, con segunda verificación:** `modulo_firma_electronica` (infraestructura) — se revisaron 6 vías adicionales (repos GitHub, imágenes Docker de terceros, release notes, mantenedor comercial, guías gubernamentales, foros) sin encontrar ninguna cifra citable más allá de la ya conocida ausencia de dato en la fuente oficial. Ver Sección 3.
- **Sigue abierto, con segunda verificación:** costo de implementación/capacitación en las 6 categorías — se reintentó con nuevos ángulos de búsqueda (marketplaces freelance, casos de gobierno abierto en México/Uruguay, foros de comunidad, consultoras especializadas) sin encontrar ninguna cifra de horas citable y aplicable a un municipio pequeño. Se descartaron explícitamente dos hallazgos por no cumplir el estándar de la tarea (ver Secciones 2 y 4). Detalle por componente en cada sección correspondiente.
...
**Costo de implementación:** `[NO VERIFICADO]`. Lo que sí tiene costo real de horas de trabajo es adaptar el código de la aplicación de trámites para cifrar/descifrar los campos de datos personales (funciones `pgp_sym_encrypt`/`pgp_sym_decrypt`, entre otras) y capacitar al personal en tratamiento de datos personales — pero ese trabajo depende por completo del sistema de trámites existente de cada municipio (variable, no estandarizable) y no se encontró una fuente pública que documente un número de horas típico para ese tipo de adaptación aplicado específicamente a un sistema de trámites municipal. No se fabrica la cifra. Segunda verificación (10-ago-2026, búsquedas adicionales sobre guías prácticas de cifrado de campos y marketplaces freelance): mismo resultado, se mantiene `[NO VERIFICADO]`.
...
- **Escalón recomendado (Docker Compose, 16 GB/8 núcleos) — verificado con cifra concreta (10-ago-2026):**
  - **DigitalOcean Droplet Premium AMD, slug `s-8vcpu-16gb-amd`** — match exacto de la especificación oficial (8 vCPU, 16 GB RAM, 320 GB SSD): **USD 112.00/mes**, obtenido directamente del bloque de datos de la aplicación embebido en el HTML de `https://www.digitalocean.com/pricing/droplets` (10-ago-2026) — `{"memory":16,"cpus":8,"disk":{"boot":320},...,"price":{"monthly":112},"slug":"s-8vcpu-16gb-amd"}`. Es la primera cifra en USD verificada directamente en la fuente oficial del proveedor para esta combinación exacta.
    - Equivalente MXN: 112.00 × 17.3562 = **1,943.89 MXN/mes**. Equivalente UYU: 112.00 × 40.0979 = **4,490.96 UYU/mes** (mismos tipos de cambio de la Sección 0, sin reverificar FX en esta pasada — solo se cerró el hueco de precio de VPS).
  - **Alternativa más barata, mismo proveedor ya usado en el resto del documento — Contabo "Cloud VPS 8"** (8 vCPU, 24 GB RAM — excede la especificación recomendada, no un match exacto): **EUR 14.00/mes**, verificado vía JSON-LD `Product`/`Offer` embebido en `https://contabo.com/en/vps/cloud-vps-core-8` (10-ago-2026). **Advertencia honesta:** a diferencia de "Cloud VPS 4" (que sí publica un `AggregateOffer` multi-moneda con USD), este plan solo publica el precio en EUR en la fuente oficial — convertirlo a USD/MXN/UYU exigiría asumir una tasa EUR-USD que este documento no fijó en la Sección 0 (que solo cubre USD-MXN y USD-UYU); se reporta la cifra en EUR tal cual, sin inventar una conversión.
  - Se intentó también Hetzner Cloud (CPX42 8vCPU/16GB y CCX33 8vCPU dedicados/32GB) — descartado como fuente verificable: el precio se carga vía un componente que llama a una API en el cliente, no está embebido en el HTML ni en JSON-LD de `hetzner.com`; solo existe una cifra de un agregador de terceros (no la fuente primaria), que no se reporta como verificada por el mismo criterio que ya aplica este documento a otras advertencias de fuente secundaria.

**Costo de implementación:** `[NO VERIFICADO]`, confirmado tras una segunda búsqueda (10-ago-2026) con nuevos ángulos (ES/EN, consultoras especializadas, foros de casos de uso, marketplaces freelance). Se encontró y se **descarta explícitamente** un dato de un vendor mexicano de gestión documental distinto a Mayan EDMS (`hdmex.com.mx`, "la metodología se ejecuta en 3 días por área a integrarse") — no cumple el estándar de esta tarea porque (a) es de un producto competidor, no de Mayan EDMS; (b) es material de marketing propio sin caso de cliente real citado; (c) no especifica si el tamaño de institución de referencia es comparable a un municipio pequeño. No se fabrica la cifra a partir de ese hallazgo no aplicable.
...
**Segunda verificación (10-ago-2026), mismo resultado:** se revisaron 6 vías adicionales sin encontrar ninguna cifra concreta: (1) `Dockerfile` de `esig/dss-demonstrations` (`github.com/esig/dss-demonstrations/blob/master/Dockerfile`) — solo build Maven + `EXPOSE 8080`, sin flags de memoria ni límites de recursos; (2) issues de ambos repos con términos "memory"/"RAM"/"heap" — sin resultados relevantes; (3) imagen Docker comunitaria de terceros `ninjaneers/dss` (Docker Hub) — README sin `mem_limit` ni flags JVM; (4) espejo de documentación del mantenedor comercial Nowina Solutions (`dss.nowina.lu/doc/dss-documentation.html`) — repite palabra por palabra el mismo texto genérico de la fuente oficial; (5) release notes de DSS (wiki de la Comisión Europea) — solo changelog de funcionalidad, sin sección de infraestructura; (6) búsqueda de guías gubernamentales (inglés/francés/español) y foros con reportes de producción — sin ningún resultado aplicable. El tracker oficial (JIRA) está detrás de login CAS de la Comisión Europea, no accesible sin autenticación. Conclusión: el hueco es estructural (la fuente oficial y su mantenedor comercial deliberadamente no publican una cifra), no un dato que una búsqueda más profunda vaya a encontrar — se mantiene `[NO VERIFICADO]`.

**Costo de implementación:** `[NO VERIFICADO]`, confirmado tras la misma segunda búsqueda — tampoco se encontró ningún caso de gobierno municipal en México/Uruguay ni ninguna fuente pública de horas típicas de despliegue + configuración de listas de confianza + capacitación para este servicio en un contexto municipal.
...
**Segunda verificación (10-ago-2026):** se encontraron dos referencias adicionales, ambas descartadas por el mismo motivo — no aplicables a un municipio pequeño: un paquete "Keycloak – 3 weeks Implementation" de Apexon (Microsoft/Azure Marketplace) orientado a clientes enterprise (banca, salud, e-commerce), sin desglose de alcance/entregables verificable; y un servicio de "análisis de instancia Keycloak" de Cloud-IAM (`cloud-iam.com/consulting`) de 60-90 minutos, que es solo una revisión diagnóstica inicial, no una implementación completa. Ninguno cumple el estándar de esta tarea — se mantiene `[NO VERIFICADO]`.
...
**Costo de implementación:** `[NO VERIFICADO]`. No se encontró en la documentación oficial de X-Road (guías de instalación, manual de usuario del Security Server) ninguna estimación de horas típicas de instalación + registro del organismo en la capa de interoperabilidad + capacitación del enlace técnico designado. No se fabrica la cifra. Segunda verificación (10-ago-2026): se revisó también X-Road Academy (cursos oficiales gratuitos por rol, con examen de certificación) — ofrece formación pero sin duración en horas/días publicada; mismo resultado, se mantiene `[NO VERIFICADO]`.
...
**Costo de implementación:** `[NO VERIFICADO]`. El trabajo real (configurar el adaptador para el proveedor de pago elegido por la intendencia, habilitar la cuenta bancaria institucional para cobros electrónicos, capacitar a caja/tesorería) depende por completo del sistema de trámites existente de cada municipio y del proveedor de pago que finalmente contrate — no hay un número de horas estandarizable ni una fuente pública que lo documente para este caso de uso genérico. No se fabrica la cifra. Segunda verificación (10-ago-2026): se encontró un listado de Upwork con precio fijo (USD 1,000) para integración de pasarela de pago en Django, pero el acceso al detalle de alcance/horas devolvió error 403 (requiere sesión autenticada) — no verificable, y aun si lo fuera, un precio fijo sin horas explícitas no cumple el estándar de esta tarea (no se extrapola horas desde una tarifa). Se mantiene `[NO VERIFICADO]`.
...
|---|---|---|---|
| `modulo_cifrado_datos` | 0 (verificado) | 0, no aplica (verificado) | `[NO VERIFICADO]` (2ª verificación 10-ago-2026) |
| `gestor_expediente_electronico` | 0 (verificado) | USD 6.60/mes piso mínimo + **USD 112.00/mes escalón recomendado** (ambos verificados, 10-ago-2026) | `[NO VERIFICADO]` (2ª verificación 10-ago-2026) |
| `modulo_firma_electronica` | 0 (verificado) | `[NO VERIFICADO]` (fuente oficial no da cifra; confirmado tras 2ª verificación exhaustiva 10-ago-2026, hueco estructural) | `[NO VERIFICADO]` (2ª verificación 10-ago-2026) |
| `identidad_federada` | 0 (verificado) | USD 6.60-24.00/mes (verificado) | `[NO VERIFICADO]` (2ª verificación 10-ago-2026) |
| `conector_interoperabilidad` | 0 (verificado) | USD 6.60/mes (verificado) | `[NO VERIFICADO]` (2ª verificación 10-ago-2026) |
| `adaptador_pasarela_pago` | 0 (verificado) | 0, no aplica (verificado) | `[NO VERIFICADO]` (2ª verificación 10-ago-2026) |

**Conclusión (actualizada 10-ago-2026):** las 6 categorías cierran el costo de licenciamiento (0, por ser OSS — hecho verificado, no hueco). El costo de infraestructura ahora cierra completo en **5 de 6 categorías** — `gestor_expediente_electronico` ya tiene ambos tiers (mínimo y recomendado) verificados con cifra concreta en USD (Sección 2); solo `modulo_firma_electronica` queda con el costo de infraestructura como hueco honesto, y una segunda pasada de verificación (6 vías adicionales: repositorios GitHub, imágenes Docker de terceros, mantenedor comercial, release notes, guías gubernamentales, foros) confirma que es un hueco **estructural** — la fuente oficial de DSS y su mantenedor comercial deliberadamente no publican una cifra de dimensionamiento, delegando al requisito genérico de la JVM.

El costo de implementación/capacitación se reintentó con una segunda búsqueda más amplia (marketplaces freelance, casos de gobierno abierto en México/Uruguay, consultoras especializadas, foros de comunidad) y **sigue `[NO VERIFICADO]` en las 6 categorías** — no existe, en ninguna fuente pública consultada, una estimación de horas de despliegue específica por componente aplicable a un municipio pequeño. Se encontraron y se descartaron explícitamente varios hallazgos cercanos por no cumplir el estándar de esta tarea (ver Secciones 2 y 4 para el detalle de por qué cada uno se rechazó). Emparejar una tarifa de mercado suelta con una cifra de horas inventada habría producido un número con apariencia de precisión pero sin fundamento real — este hueco se deja como pendiente de dato explícito para una futura iteración, idealmente alimentado con datos reales del piloto (horas efectivamente invertidas) en vez de una estimación externa, tal como ya anticipaba la primera versión de este documento.
