...
Versión 1 · 6 de agosto de 2026. Cierra una brecha de producto detectada al revisar el flujo de adopción: no existe hoy ningún mecanismo para dar de alta el primer gobierno (tenant) y su primer usuario en un despliegue nuevo, y ese paso no puede ser frágil en un proyecto open source pensado para gobiernos.

Este documento es **solo planeación**. Ninguna tarea que aquí se describe se ha ejecutado. Criterio rector explícito de este diseño: **no fragilidad operativa por encima de elegancia de código** — cada alternativa se evalúa contra ese criterio, no contra facilidad de implementación.

## 0. La brecha exacta (verificada contra el código real del repositorio)

- `backend/app/seed.py` (archivo completo) es un fixture de desarrollo explícito: docstring líneas 1-8 ("Fixture de desarrollo/pruebas únicamente — nunca fuente de verdad para datos de un municipio o intendencia real"); línea 17 fija `PASSWORD_PRUEBA = "cambiar123"` (pública en el repo, compartida por los dos usuarios de prueba); líneas 27-32 abortan con `RuntimeError` si `settings.environment == "production"`. Ese guard es parte del diseño original de la tarea B4 (`docs/plan-implementacion.md` línea 26: "`app/seed.py` aborta si `ENVIRONMENT=production` para no crear el usuario de password fijo fuera de dev/test") y no se toca aquí — es precedente de diseño, no algo a rediscutir.
...
(b) se descarta explícitamente: agrega justo la clase de fragilidad que el criterio rector busca evitar (un secreto nuevo que gestionar/perder/olvidar desactivar, un estado "usado" que puede desincronizarse de la realidad tras un reinicio) a cambio de una ventaja que el usuario objetivo (contraparte técnica con acceso al servidor, `docs/PRD.md` línea 44) no necesita. (c) queda como posibilidad futura explícita, no cerrada por este diseño (la función interna del CLI puede reutilizarse detrás de un endpoint más adelante si un despliegue centralizado sin shell lo justifica) pero no se construye ahora.
...
- **`[NO VERIFICADO]`:** si en el futuro se operara un despliegue centralizado para múltiples gobiernos sin que cada uno tenga acceso de shell al servidor (en cuyo caso la alternativa (c) dejaría de ser opcional), no hay evidencia en la documentación del producto de que ese escenario esté decidido — se trata como hipótesis futura, no como requisito actual.

## 8. Tabla de tareas de construcción

**Actualizado 2026-08-11 — corrección de estado desactualizado (mismo tipo de desfase ya corregido antes para `docs/plan-implementacion-e1-bis-capa-ia-local.md`, commit `798911d`): las cuatro tareas ya estaban construidas y en uso, la tabla seguía diciendo "Pendiente".** Verificado contra el código real, no contra memoria: `backend/app/bootstrap_tenant.py` (201 líneas, commit `e062aac`, 2026-08-06) implementa `crear_gobierno`/`resetear_password` exactamente como se diseñó en las secciones 1-6; `backend/app/seed.py` ya reutiliza `crear_gobierno` en vez de construir `Tenant`/`Usuario` por su cuenta (ver docstring de `bootstrap_tenant.py` línea 3); el runbook de despliegue y el job `e2e` de CI (`.github/workflows/ci.yml`) ya invocan `python -m app.bootstrap_tenant crear-gobierno` contra un stack Docker real para levantar el gobierno de prueba de cada corrida.

| # | Tarea | Depende de | Bloquea | Estado |
|---|---|---|---|---|
| ALTA-1 | Implementar `backend/app/bootstrap_tenant.py` (subcomandos `crear-gobierno` y `resetear-password`, `argparse`), reutilizando `app/db/rls.py` (sección 6), `app/core/security.py` (`hash_password`) y un generador de contraseña nuevo (`generar_password_legible()`, propuesto en `app/core/security.py` junto a `hash_password`/`verify_password`, sección 2) | Ninguna | ALTA-2 | Hecho |
| ALTA-2 | Tests unitarios: idempotencia (clave duplicada → no-op limpio), validaciones de entrada (sección 5), transaccionalidad/rollback (sección 4), uso real de `fijar_contexto_tenant`/`abrir_sesion_tenant` (no una copia local, sección 6), entropía/alfabeto del generador de contraseña, `resetear-password` sobre un usuario existente y sobre uno inexistente — `backend/tests/test_bootstrap_tenant.py` | ALTA-1 | ALTA-3 | Hecho — 13 tests, cubre las 6 dimensiones listadas (idempotencia contra Postgres real, validaciones de país/email/clave/nombre, alfabeto/entropía/no-repetición del password, resetear-password sobre usuario y tenant inexistentes) |
| ALTA-3 | Documentación: sección nueva "Alta de un gobierno nuevo" en `docs/TRD.md`; runbook operativo nuevo `docs/runbook-alta-gobierno.md` con el comando exacto (`docker compose exec backend python -m app.bootstrap_tenant ...`) y la captura de pantalla/ejemplo de salida esperada; confirmar explícitamente en `.env.example` que este mecanismo no agrega ninguna variable nueva (sección 1, tabla) | ALTA-1 | ALTA-4 | Hecho |
| ALTA-4 | Revisión integral de ALTA-1 + ALTA-2 + ALTA-3 como un solo entregable (código + tests + docs, un solo ciclo de revisión) | ALTA-3 | Cierre de este plan | Hecho de facto, no como ciclo de revisión formal separado — el mecanismo se verificó de punta a punta en uso real: `docker compose exec backend python -m app.bootstrap_tenant crear-gobierno` corrido contra un stack Docker real (2026-08-07, gobierno de prueba `prueba-qa`) y en cada corrida del job `e2e` de CI. No hubo un commit dedicado exclusivamente a "revisión" — quedó integrado en el mismo commit `e062aac` junto con otros cambios, desviación menor de la secuencia planeada sin impacto funcional. |

**Paralelismo real disponible:** ninguno — las tres tareas de construcción son estrictamente secuenciales (ALTA-2 necesita el código de ALTA-1; ALTA-3 necesita el comando final de ALTA-1 para documentarlo con precisión; ALTA-4 necesita las tres).
...
- El nombre exacto de la función `generar_password_legible()` y su ubicación en `app/core/security.py` es una propuesta de este plan, no un contrato ya fijado — la implementación puede ajustar el nombre siempre que preserve las propiedades de la sección 2 (alfabeto sin ambigüedad, ≥ 92 bits de entropía).
