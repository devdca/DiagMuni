...
| Capa IA | LiteLLM sirviendo **DeepSeek vía API** (F1 captura, F9 verificador — tareas generales de bajo costo) y **Claude vía API** (F3 generación compleja del plan); degradación a plantillas deterministas si la API no responde | El wiring del modelo local cuantizado (candidato evaluado: phi3/Phi-3-mini, licencia MIT) ya está construido: ruta `local` en `backend/app/ia/litellm_config.yaml` y resolución de proveedor en `backend/app/ia/config.py` (`obtener_proveedor_llm()`). Sin `LLM_PROVIDER` fijado explícitamente, el default real es `local` si `OLLAMA_API_BASE` está poblada (y plantilla determinista si no); el operador que quiere Claude/DeepSeek en producción debe fijar `LLM_PROVIDER=anthropic` o `=deepseek` a propósito — ver "Nota sobre la Capa IA" más abajo |
...
| Observabilidad | Logs a stdout + endpoint `/health` (vigilado por `backend/scripts/vigilar_salud.py`, ver `docs/runbook-despliegue.md`) + log de auditoría del diagnóstico + captura opcional de errores 5xx (`SENTRY_DSN`, backend usa `sentry-sdk`, BSD-2-Clause -- protocolo compatible con Sentry, no una dependencia de Sentry mismo) | Sin stack pesado (sin Grafana/Prometheus/OTel) para el MVP. Captura de errores opcional a propósito (sin `SENTRY_DSN` no se inicializa nada). Camino recomendado: **GlitchTip** autohospedado (MIT, `docker-compose.yml` perfil `observabilidad`) -- a diferencia del Sentry self-hosted oficial (BSL/FSL, no Apache/MIT/BSD puro), GlitchTip sí cumple el Principio 2 de este documento sin necesitar el carve-out de "recomendación de catálogo, no dependencia integrada". sentry.io (SaaS) sigue siendo una alternativa válida para quien prefiera no autohospedar -- mismo SDK, solo cambia el DSN |
...
**Nota sobre LiteLLM (licencia):** verificado directamente contra el repositorio oficial (`github.com/BerriAI/litellm`, archivo `LICENSE` en la raíz) — **MIT**, sin condiciones adicionales. El repositorio separa licencias por directorio: el contenido bajo `enterprise/` (paquete opcional `litellm-enterprise`, funciones de proxy empresarial: SSO, facturación, licenciamiento de asientos) está bajo una licencia propia y distinta, declarada explícitamente vía `enterprise/LICENSE.md` (el SPDX `LicenseRef-Proprietary` se fija en `enterprise/pyproject.toml`, campo `license`) — no es ambigüedad, es una separación clara. DiagMuni no usa ni importa ese directorio: el uso previsto es exclusivamente el SDK core (`litellm.completion` / `litellm.Router`), 100% MIT. Se deja esta nota por escrito antes de que E2 agregue `litellm` a `requirements.txt`, siguiendo el mismo estándar de verificación aplicado al resto del catálogo OSS en este documento.

**Nota — `gestor_expediente_electronico`:** `entregables/fase-2/modelo-diagnostico.md` declara que la digitalización del expediente requiere "adoptar un gestor de expediente electrónico (categoría, no marca)". Esta categoría no tiene fila en la tabla de arriba (es de un eje distinto a "Atención Ciudadana") pero ya está resuelta: `entregables/fase-2/catalogo-componentes-oss.md` (sección 2) verificó y eligió **Mayan EDMS** (Apache-2.0, licencia confirmada por lectura directa del archivo `LICENSE` del repositorio oficial, actividad reciente confirmada) — ya no queda pendiente.
...
| Modelo local único (phi3/Phi-3-mini, MIT) como default de producción de Capa IA | Evaluado a fondo (dos rondas de benchmark) y sin ningún problema de licencia, pero **no adoptado como default**: una prueba comparativa real mostró la API ~20-40x más rápida (~4.7s vs. 101-194s por párrafo) y de costo marginal insignificante para el volumen de un piloto (~USD 0.0005-0.001 por llamada de F3, según franja horaria peak/off-peak) — ver `entregables/fase-2/dimensionamiento-costos.md`. Sigue disponible vía cambio de configuración de LiteLLM (sin tocar `engine/`) si en el futuro cambia el criterio (presupuesto de API, política de datos, o necesidad de operar sin conexión a internet). Aclaración de términos: "no adoptado como default" se refiere aquí a la arquitectura de producción documentada del piloto (DeepSeek+Claude vía API) -- `local` sí es el default técnico automático que el código resuelve cuando el operador no fija `LLM_PROVIDER` explícitamente (ver "Nota sobre la Capa IA" más abajo); son dos sentidos distintos de la palabra "default", no una contradicción. |
...
## Nota sobre la Capa IA — modelo local evaluado, no elegido como arquitectura de producción documentada (sí es el default técnico automático sin configurar)

**Decisión:** DeepSeek + Claude vía API como arquitectura de producción documentada, con degradación a plantillas deterministas si la API no responde. El wiring de código del modelo local cuantizado (phi3/Phi-3-mini, MIT) ya está construido — no es una alternativa manual futura: `backend/app/ia/litellm_config.yaml` define la ruta `local`, y `backend/app/ia/config.py` (`obtener_proveedor_llm()`) resuelve, sin `LLM_PROVIDER` fijado explícitamente, `local` como default si `OLLAMA_API_BASE` está poblada (plantilla determinista si no). El operador de cada despliegue que quiera usar Claude/DeepSeek en producción debe fijar `LLM_PROVIDER=anthropic` o `=deepseek` a propósito: el operador elige el proveedor sin tocar código, y la plataforma nunca gasta en una API de pago sin que se lo pidan explícitamente.

**E1bis-2 cerrado:** la instalación real de Ollama (`ollama pull phi3`) y la prueba de integración end-to-end sin mocks contra un Ollama corriendo de verdad ya se ejecutaron. `backend/tests/test_generador_plan_ollama_real.py` (skip limpio si no hay Ollama alcanzable con `phi3` descargado, mismo patrón que `_postgres_real_disponible()` de `test_api_seguimiento.py`) corrió dos veces contra un Ollama real: 76.38s y 57.8s, con narrativa capturada y confirmada distinta de la plantilla determinista.

**Por qué:** un benchmark real (mismo párrafo de prueba F3, "Firma electrónica", generado por ambos caminos) midió la API de DeepSeek en 4.7 segundos, contra 101-194 segundos de los mejores candidatos locales con licencia limpia — una diferencia de un orden de magnitud en latencia, con costo marginal insignificante para el volumen de un piloto (~USD 0.0005-0.001 por llamada; cifra y modelo exacto verificados 18-ago-2026, detalle y metodología en `entregables/fase-2/dimensionamiento-costos.md` sección 8). El diseño de LiteLLM como capa de abstracción hace que este default sea reversible por configuración, no una decisión estructural: cambiarlo más adelante no exige reescribir `ia/` ni `engine/`.
