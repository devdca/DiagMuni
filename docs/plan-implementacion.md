...
Sexto y último de los 6 documentos de blueprint de producto. Secuencia exacta de construcción del código, paso a paso, con dependencias explícitas — para que la implementación sepa en qué orden construir, no solo qué construir. Este documento es el plan de construcción del *código*, distinto del cronograma de la postulación del proyecto.

## Principio de secuencia: determinista primero, IA después — también en el orden de construcción

No solo en runtime — el motor determinista se construye y se valida **completo, de punta a punta, sin ninguna llamada a LLM**, antes de agregar la capa de IA (fases A-D antes que fase E). Esto obliga a probar en código, no solo en teoría, que el producto tiene valor sustantivo sin IA (ver el ejercicio de la tabla brecha→acción sin LLM, discutido y validado antes de escribir código) — si algo no funciona sin IA en la fase D, agregar IA en la fase E no lo arregla, lo disfraza.
...
**Punto de control obligatorio al final de la fase C**: generar un plan completo para un trámite de prueba usando solo C1-C3, sin ninguna key de API configurada, y verificar que es sustantivamente útil (mismo ejercicio ya validado manualmente antes de escribir código, ahora en código real).
