...
- **Reenvío mientras el plan se está generando**: el envío responde `409` y no guarda las respuestas nuevas — guardarlas dejaría al plan en curso describiendo un diagnóstico que ya cambió, y aceptarlas en silencio le haría creer al funcionario que quedaron guardadas. La pantalla muestra el motivo y el funcionario reenvía cuando el plan termina; su captura no se pierde. Si el job vigente quedó colgado (proceso reiniciado a medio camino), el reenvío no da `409`: redispara ese mismo job.
- **Cooldown del envío**: `POST /diagnostico/enviar` es la única operación con costo real de LLM, así que tiene un límite propio — por trámite (acota regenerar el plan del mismo trámite) y un techo por usuario muy por encima de la captura manual (acota un script que recorra muchos trámites). Capturar el catálogo completo de un municipio, un envío por trámite, nunca lo topa.
