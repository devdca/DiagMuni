...
### Revisión de diseño — ronda 1 (dentro del brief, ningún principio relajado)

Antes de la ronda 2 de abajo hubo una primera pasada de jerarquía visual, explorada también como mockups antes de tocar código, que no requirió permiso para romper ninguna regla porque no rompe ninguna: sin animación, sin imagen, sin color nuevo. Se documenta aquí solo para que el historial de qué cambió en cada pantalla quede completo (mismo espíritu que el resto de esta sección), no porque haya tensión con ningún principio.

- **`PageHeader.tsx` / `IconChip.tsx`:** un rótulo pequeño ("kicker") sobre el título y un ícono de línea propio por sección, en tinta neutra — nunca el azul de la rampa de madurez ni el navy del navbar (principio 5). En uso en Panel resumen (título de página) y Plan de modernización (título de tarjeta, versión más chica del mismo componente).
- **`GraficaTendenciaIndice.tsx`:** cuando hay menos de 2 diagnósticos, el texto "sin datos suficientes" ahora lleva junto un motivo de línea institucional estático (`aria-hidden`, tinta neutra) — nunca reemplaza el texto, solo lo acompaña, mismo criterio que el resto de la interfaz de "color/ícono nunca solo".
- **`Plan.tsx` — `ResumenPlanRail`:** panel lateral fijo (`lg:sticky`) con el resumen del propio plan — total de brechas, brechas con componente identificado, inversión única y costo recurrente estimados. No es un dato nuevo: son los mismos valores de `resumen_inversion` que ya muestra `TarjetaPresupuesto` en la pestaña "Resumen ejecutivo", solo visibles sin cambiar de pestaña. El mockup original de esta pieza asumía prioridad y costo por brecha individual; se ajustó a lo que el backend (`backend/app/engine/resumen_plan.py`) sí calcula, en vez de inventar esos campos.

### Revisión de diseño — ronda 2 (autorizada por Ricardo, 15 de septiembre de 2026)

Feedback de producto tras ver la primera implementación: "se ve muy sencilla, necesito que se vea más bonita, moderna, atractiva". Se exploraron 3 direcciones como mockups (canvas de diseño, no código) antes de tocar nada — una editorial (solo tipografía), una de tarjetas con más profundidad/sombra, y una con una franja de color e ilustración. Ricardo autorizó explícitamente relajar puntos concretos de los principios 2 y 5 (pantalla 1) si el resultado se veía bien, con una condición: documentarlo aquí, igual que ya se documenta el historial de las 2 paletas descartadas arriba. Después afinó el criterio de éxito a algo específico, no "lo que sea que se vea más lleno": **minimalista, profesional, "gobierno digital"** — la referencia que dio fue GOV.UK / e-Estonia / servicios digitales nórdicos — serio pero no frío, simple pero no plano, con calidez pero sin caer en lo decorativo o maximalista.

Lo que cambió, puntual a la franja del índice global en el Panel resumen (`frontend/src/components/HeroIndiceGlobal.tsx`):

- **Principio 2 ("sin animaciones decorativas"):** excepción puntual, no un cambio general. La cifra del índice y su mini-gráfica de tendencia entran con una transición de una sola vez al cargar el dato real (nunca en loop, respeta `prefers-reduced-motion` sin excepción — ver `.hero-indice-cifra` / `.hero-spark-linea` en `frontend/src/index.css`). Es una entrada de contenido, no un adorno; el resto de la interfaz sigue sin ninguna animación de este tipo.
- **Pantalla 1 ("única pantalla con imagen"):** el Panel resumen suma una franja con fondo oscuro fijo (`--hero-bg`, mismo criterio de "color de identidad propia, no theme-aware" que ya usa el navbar) y una marca de línea propia (`IlustracionAyuntamiento.tsx`) — deliberadamente **no** es una fotografía ni una escena decorativa: es el mismo lenguaje de ícono de trazo que ya usa el resto de la app, escalado a tamaño de marca. El login sigue siendo la única pantalla con fotografía de fondo; esta es una excepción distinta y más acotada.
- **Un solo acento de color, nunca arcoíris:** el único color nuevo en la franja es el dorado que ya existía en el navbar (línea del link activo, anillo del avatar) — no se inventó un tono nuevo, se le dio un segundo uso deliberado y mínimo (la bandera del ícono). La cifra del índice sigue pintándose con la rampa ordinal ya validada (variante `varTextoSobreOscuro` en `madurez.ts`/`index.css`, mismos 5 hex del modo oscuro, pensada para una superficie que siempre es oscura) — sigue siendo el único color con significado propio de la interfaz.
- **Lo que no cambió:** ninguna otra pantalla lleva franja hero, imagen ni animación de esta ronda — Cuestionario, Seguimiento y Perfil del gobierno no tuvieron ningún cambio (ni siquiera los de ronda 1); Plan sí tiene los cambios de ronda 1 (ícono de sección, panel lateral), pero esos no rompen ningún principio, son un pase distinto y anterior (ver arriba). Esto no es un rediseño general, es una pieza acotada del Panel resumen. Si se decide extender el mismo lenguaje (franja/animación/acento dorado) a otras pantallas, actualizar esta sección con esa decisión también.
...
Única pantalla de toda la app con fotografía de fondo (`frontend/public/branding/login-fondo.png`, imagen propia) — momento de bienvenida antes de entrar, no una superficie de trabajo; el resto de las pantallas se queda en superficie plana (sección "Paleta"), con una excepción puntual y distinta en el Panel resumen (ver "Revisión de diseño — ronda 2" arriba: una marca de línea propia, no una fotografía). La imagen lleva un velo neutro oscuro (no azul, para no reintroducir "todo azul" en la puerta de entrada) y la tarjeta de login es glassmorphism **deliberadamente opaco, no transparente** (`--card` al ~92% + blur) — se lee como vidrio sobre la foto sin depender de lo que haya detrás para mantener el contraste AA ya validado de `--card`.

### 2. Panel resumen
Franja "hero" superior con el índice de madurez global (cifra grande + etiqueta, paleta ordinal de arriba, entrada animada de una sola vez) y fecha de último diagnóstico — ver "Revisión de diseño — ronda 2" arriba para el porqué de esta pieza. Debajo, una gráfica de tendencia apilada: cuántos trámites activos hay en cada nivel de la rampa (0-4) en el tiempo, área por nivel con los mismos 5 colores ya validados de esa rampa (`frontend/src/lib/madurez.ts`) — nunca colores nuevos ni series inventadas para "verse más llena". Debajo, tabla de trámites catalogados con su índice individual (`Badge`) y acceso a "continuar diagnóstico" o "ver plan".
