...
def citas_y_numeros_son_fieles(narrativa: str, brecha_determinista: dict, contexto_gobierno: str = "") -> bool:
    ...
    `brecha_determinista` (ver `_CAMPOS_REFERENCIA`) o en `contexto_gobierno`
    (app/ia/contexto_gobierno.py -- mismo texto exacto que ya vio el prompt de
    redacción, para no rechazar una cifra real del perfil del gobierno solo por
    no estar en los 5 campos fijos de esta brecha en particular). False si la
    narrativa introduce al menos un identificador que no está en ninguno de los
    dos -- fail-closed, igual que el resto de F9: ante duda se rechaza, nunca se
    aprueba por omisión."""
